        ...
        if custom_prompt:
            ...
            if product_info:
                ...
                if title or features or badge:
                    ...
                    if title:
                        text_section += (
                            f"НАЗВАНИЕ ТОВАРА: Сверху изображения ЗАГЛАВНЫМИ БУКВАМИ — '{title.upper()}'. "
                            "Белый жирный шрифт 48-72pt на тёмном фоне.\n"
                        )
                    ...
                    if features:
                        ...
                        for feat in features:
                            ...
                            if match:
                                number_parts.append(
                                    f"Число '{match.group(1)}' с подписью '{match.group(2)}' "
                                    f"ОГРОМНЫМ шрифтом 72-96pt (из: '{feat}')"
                                )

                        if number_parts:
                            text_section += (
                                "КЛЮЧЕВЫЕ ЦИФРЫ (главный визуальный элемент, КРУПНЕЕ названия):\n"
                            )
                            for np_item in number_parts:
                                text_section += f"  - {np_item}\n"

                        text_section += (
                            "ПРЕИМУЩЕСТВА в полупрозрачных плашках (pill-shape, скруглённые "
                            "прямоугольники с rgba(255,255,255,0.1) фоном):\n"
                        )
                        icons = ["⚡", "🛡️", "💧", "✓", "🔥"]
                        positions = [
                        ...
                        for i, feat in enumerate(features):
                            ...
                            text_section += f"  - {icon} '{feat}' — плашка {pos} от товара\n"

                    if badge:
                        text_section += (
                            f"БЕЙДЖ: '{badge}' — яркий цветной кружок/лента "
                            "в верхнем правом углу. Белый жирный текст.\n"
                        )

                    text_section += (
                        "\nВсе тексты на РУССКОМ ЯЗЫКЕ. Тёмный фон обязателен. "
                        "Белая типографика, полупрозрачные плашки с иконками. "
                        "Крупные цифры — ГЛАВНЫЙ визуальный элемент. "
                        "8K фотореализм, стиль продающей карточки Wildberries/Ozon.\n"
                    )
